...
const NavbarView: AdminView = ({ user, canAccessAdmin }) => {
    ...
    <main className="flex min-h-screen flex-col items-center justify-between p-0">
      <div className="navbar bg-base-100 px-8">
        <div className="flex-1">
          <img src={data.logo.url} width={40} height={40} alt="SCSE logo" />
          <a className="btn btn-ghost text-xl">{'SCSE'}</a>
        </div>
...
const NavItem = ({ navData }) => {
  return (
      <details>
        ...
        { navData.subnavItems && <ul className="p-2 bg-base-200 rounded-t-none">
...
const FooterView = ({ footerData }) => {
  ...
  return (
    <footer className="footer p-10 bg-neutral text-neutral-content w-1/2">
      {
        data.map(dataCol => (
          <nav>
            <h6 className="footer-title">{dataCol.title}</h6>
            {dataCol.itemList.map( item => (<a className="link link-hover">{item}</a>) )}
          </nav>
        ))
      }
    </footer>
